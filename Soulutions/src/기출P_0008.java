...
// [기출P-0008] 연방
// 교류를 하는 도시인지 아닌지 입력된 처리 또는 질문에 따라 답을 생성하여 출력
// 모든 교류를 차단하는 간선을 처리하여 거꾸로 교류여부를 체크하면서 차단된 교류를 다시 연결하는 방법으로 거꾸로 체크하면서 출력할 결과물을 생성한다.
		...
		for(int tc = 1; tc <= T; tc++ ) {
			...
			for(int i = Q - 1; i >= 0 ; i--) { // 배열을 거꾸로 채워넣고 이후 출력도 거꾸로 생성 한다.
				...
				if(1 == Integer.parseInt(st.nextToken())) { // 간선의 교류 정보를 제거하는 처리의 경우  
					int k = Integer.parseInt(st.nextToken()); // 입력된 간선 정보중 몇번째 교류 정보를 제거할지 체크
					cutOff[k] = true;
					q[i][0] = 1;
					q[i][1] = k;
